...
class RandomBitGeneratorTest < Test::Unit::TestCase
  ...
  def test_random_bit
    counts = generate_random_bits(1000)
    assert_greater_than counts[0], 400
    assert_greater_than counts[1], 400
  end
  ...
  def test_random_bit_with_typical_parameters
    counts = generate_random_bits(1000, 0.2)
    assert_greater_than counts[0], 700
    assert_greater_than counts[1], 100

    counts = generate_random_bits(1000, 0.9)
    assert_greater_than counts[0],  50
    assert_greater_than counts[1], 850
  end
